...
interface FriendshipRepository : JpaRepository<Friendship, Long> {
    ...
    LEFT JOIN FETCH r.profile rp 
    WHERE (s.userId = :userId OR r.userId = :userId) 
    AND f.status = 2
"""
    )
    fun findAllFriendsWithProfiles(@Param("userId") userId: Long): List<Friendship>
...
"""
    ...
    fun findPendingWithProfiles(@Param("userId") userId: Long): List<Friendship>
}